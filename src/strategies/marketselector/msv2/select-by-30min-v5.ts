...
/**
 * Short
 * To do that we will invert candles. Red will become green and green become red
 */
export class SelectBy30minV5 {
     ...
     *    | <-- big negative variation (c2)
     *     | <-- big negative variation (c1) with a big volume (v1)
    ...
    static shouldSelectMarket(state: MountainSeekerV2State, market: Market,
        ...
        if (market.percentChangeLast24h! < -35 || market.percentChangeLast24h! > 35) {
            return undefined;
        }

        let candlesticksCopy = cloneDeep(candleSticks);
        let candleSticksPercentageVariationsCopy = cloneDeep(candleSticksPercentageVariations);
        let past = false;
        let secondsToSleep;
        ...
        // open remains open, low becomes high, high becomes low, close becomes open-close
        candlesticksCopy = candlesticksCopy.map(candle => [candle[0], candle[1], candle[3], candle[2], candle[1]-candle[4], candle[5], candle[6]]);
        candleSticksPercentageVariationsCopy = candleSticksPercentageVariationsCopy.map(variation => -variation);
